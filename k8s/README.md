...
for s in service-discovery config-service apigateway-service product-service commerce-service recommendation-service; do
  docker tag repo-$s zeropick/$s:local   # compose 프로젝트명에 따라 접두어 확인
done
